...
// 调用 write 的时候，实际的音频数据格式会根据 Spec 自动转换为存储所需的音频格式。
// 且每次调用的时候，都会自动调用 check_channels 来判断写入的声道数对不对，如果不对就会返回错误值。
...
pub trait AudioWriter {
    ...
    fn check_channels(&self, frame: &Vec<f64>) -> Result<(), Box<dyn Error>> {
