...
pub mod oggvorbis_enc {
    ...
    impl<'a> BitReader<'a> {
        pub fn new(data: &'a [u8], cursor: &'a usize) -> Self {
            Self {
                mask: [
        ...
        pub fn read(&mut self, mut bits: i32) -> Result<i32, AudioReadError> {
            ...
            let m: u32 = self.mask[bits];
            bits += self.endbit;

            if self.endbyte >= self.storage - 4 {
                if self.endbyte > self.storage - ((bits + 7) >> 3) {
                    return Err(AudioReadError::UnexpectedEof("UnexpectedEof".to_string()));
                } else if bits == 0 {
                    return Ok(0);
                }
            }
            let mut cursor = self.cursor;
            let mut ret: i32 = self.data[cursor + 0] >> self.endbit;
            if bits > 8 {
                ret |= self.data[cursor + 1] << (8 - self.endbit);
                if bits > 16 {
                    ret |= self.data[cursor + 2] << (16 - self.endbit);
                    if bits > 24 {
                        ret |= self.data[cursor + 3] << (24 - self.endbit);
                        if bits > 32 && self.endbit != 0 {
                            ret |= self.data[cursor + 4] << (32 - self.endbit);
                        }
                    }
                }
            }
    ...
    impl Default for StaticCodeBook {
        fn default() -> Self {
            Self {
                dim: 0,
                entries: 0,
    ...
    impl StaticCodeBook {
        pub fn read(data: &[u8], cursor: &mut usize) -> Result<Self, AudioReadError> {
            let mut bit_reader = BitReader::new(data, cursor);
            let mut ret = Self::default();
            ret.allocedp = 1;

            let num_books = bit_reader.read(8)? + 1;
            for i in 0..num_books {
                if bit_reader.read(24)? != 0x564342 {
                    return Err(AudioReadError::UnexpectedEof("Check the `BCV` flag failed.".to_string()));
                }
                ret.dim = bit_reader.read(16)?;
                ret.entries = bit_reader.read(24)?;
                if ilog(dim as u32) + ilog(entries as u32) > 24 {break;}
                match bit_reader.read(1)? {
                    0 => {
                        let unused = if bit_reader.read(1)? != 0 {true} else {false};
                        if ((ret.entries * (if unused {1} else {5}) + 7) >> 3 > bit_reader.storage - bit_reader.bytes()) {
                            return Err(AudioReadError::InvalidArguments(format!("(({} * {} + 7) >> 3 > {} - {})", ret.entries, if unused {1} else {5}, bit_reader.storage, bit_reader.bytes())));
                        }
                        ret.lengthlist.resize(entries, 0);
                        if unused {
                            for i in 0..entries {
                                if bit_reader.read(1)? != 0 {
                                    let num = bit_reader.read(5)?;
                                    ret.lengthlist[i] = num + 1;
                                } else {
                                    ret.lengthlist[i] = 0;
                                }
                            }
                    ...
                    1 => {
                        let mut length = bit_reader.read(5)? + 1;
                        ret.lengthlist.resize(entries, 0);
                        let mut i = 0;
                        while i < ret.entries {
                            let num = bit_reader.read(ilog(ret.entries - i))?;
                            if length > 32 || num > ret.entries - i || (num > 0 && (num - 1) >> (length - 1) > 1) {
                                return Err(AudioReadError::InvalidArguments(format!("{length} > 32 || {num} > {} - {i} || ({num} > 0 && ({num} - 1) >> ({length} - 1) > 1)", ret.entries)));
                            }
                            for j in 0..num {
                                ret.lengthlist[i] = length;
                                length += 1;
                                i += 1;
                            }
                        }
                    }
                    o => return Err(AudioReadError::FormatError(format!("Unexpected {o}"))),
                }

                ret.maptype = bit_reader.read(4)?;
                match ret.maptype {
                    0 => (),
                    1 | 2 => {
                        ret.q_min = bit_reader.read(32)?;
                        ret.q_delta = bit_reader.read(32)?;
                        ret.q_quant = bit_reader.read(4)? + 1;
                        ret.q_sequencep = bit_reader.read(1)?;

                        let quantvals = match ret.maptype {
                            1 => if ret.dim == 0 {0} else {self.book_maptype1_quantvals()},
                            2 => ret.entries * ret.dim,
                        };
                    }
                }
            }
